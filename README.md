...
См. `site/README.md`. Коротко: создать MySQL 8, прописать пароль в `config.php`, залить содержимое `site/` в `public_html` (**не** перезаписывая живой `config.php`). Сайт работает по **http://**; SSL можно включить позже.
