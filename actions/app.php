...
function crm_action_save_app(PDO $pdo, array $user, int $viewUid): never {
    ...
    try {
        $upd = $pdo->prepare('UPDATE crm_lead_apps SET `number`=?, city_from=?, city_to=?, rate=?, margin=?, vat=?, carrier_rate=?, carrier_vat=?, carrier_company=?, carrier_inn=?, carrier_name=?, carrier_phone=?, updated_at=? WHERE id=? AND lead_id=? AND updated_at=?');
        $upd->execute([$f['number'], $f['cityFrom'], $f['cityTo'], $f['rate'], $f['margin'], $f['vat'], $f['carrierRate'], $f['carrierVat'], $f['carrierCompany'], $f['carrierInn'], $f['carrierName'], $f['carrierPhone'], $now, $id, $leadId, (int) $app['updated_at']]);
        if ($upd->rowCount() === 0) {
            $pdo->rollBack();
            err('Заявка изменена в другом месте');
        }
