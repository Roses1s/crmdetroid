...
// --- crm_ft_query: запрос для FULLTEXT-поиска (BOOLEAN MODE) ----------------
t('ft: слова становятся обязательными префиксами', crm_ft_query('Детроид Логистик'), '+Детроид* +Логистик*');
t('ft: короткие слова (<3) выпадают, «ООО» (ровно 3) остаётся', crm_ft_query('ИП Детроид ООО'), '+Детроид* +ООО*');
t('ft: только короткие слова → пусто (fallback на LIKE)', crm_ft_query('ИП а'), '');
t('ft: операторы BOOLEAN MODE — разделители, в запрос не попадают', crm_ft_query('дет+роид (тест) "к@вычки" ~з*везда'), '+дет* +роид* +тест* +вычки* +везда*');
t('ft: пустая строка', crm_ft_query('   '), '');
