...
R=$(post "$JI" "$TI" save_lead_app "{\"leadId\":\"$LAPP\",\"cityFrom\":\"Новоград\",\"cityTo\":\"Староград\",\"rate\":\"5 000\",\"carrierCompany\":\"Смоук Транс\",\"carrierInn\":\"1234567890\",\"carrierName\":\"Пётр\",\"carrierPhone\":\"+7 (900) 111-22-33\"}"); check "v20: заявка с перевозчиком создана" "r.get('success') is True" "$R"
R=$(get "$JI" "get_directions"); DID2=$(jget "$R" "[d['id'] for d in r['directions'] if d.get('cityFrom')=='Новоград' and d.get('cityTo')=='Староград'][0]")
R=$(get "$JI" "get_carriers&id=$DID2"); check "v20: перевозчик заявки в справочнике с реквизитами" "any(c.get('company')=='Смоук Транс' and c.get('inn')=='1234567890' and c.get('name')=='Пётр' and c.get('phone')=='+7 (900) 111-22-33' and c.get('createdByName')=='$A_NAME' for c in r.get('carriers',[]))" "$R"
R=$(post "$JI" "$TI" save_lead_app "{\"leadId\":\"$LAPP\",\"cityFrom\":\"Новоград\",\"cityTo\":\"Староград\",\"rate\":\"6 000\",\"carrierCompany\":\"Смоук Транс\",\"carrierInn\":\"1234567890\",\"carrierName\":\"Пётр\",\"carrierPhone\":\"+7 (900) 111-22-33\"}"); check "v20: повторный перевозчик — заявка создана" "r.get('success') is True" "$R"
R=$(get "$JI" "get_carriers&id=$DID2"); check "v20: дубль перевозчика не создан" "sum(1 for c in r.get('carriers',[]) if c.get('inn')=='1234567890')==1" "$R"
R=$(post "$JI" "$TI" save_lead_app "{\"leadId\":\"$LAPP\",\"cityFrom\":\"Новоград\",\"cityTo\":\"Староград\",\"rate\":\"7 000\"}"); check "v20: заявка без перевозчика создана" "r.get('success') is True" "$R"
R=$(get "$JI" "get_carriers&id=$DID2"); check "v20: пустой перевозчик не создан" "len(r.get('carriers',[]))==1" "$R"
