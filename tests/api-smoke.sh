...
R=$(post "$JI" "$TI" save_lead_app "{\"leadId\":\"$LAPP\",\"cityFrom\":\"Москва\",\"cityTo\":\"Уфа\",\"loadAddress\":\"ул. Северная, 54\",\"loadContact\":\"+7 (900) 111-22-33 Пётр\",\"loadDateFrom\":\"2026-09-20\",\"loadTimeFrom\":\"09:00\",\"unloadDateTo\":\"2026-09-22\"}"); check "v23: точки погрузки сохраняются" "r.get('application',{}).get('loadAddress')=='ул. Северная, 54' and r.get('application',{}).get('loadDateFrom')=='2026-09-20' and r.get('application',{}).get('loadTimeFrom')=='09:00'" "$R"
APPLOAD=$(jget "$R" "r['application']['id']")
R=$(post "$JI" "$TI" save_lead_app "{\"id\":\"$APPLOAD\",\"leadId\":\"$LAPP\",\"cityFrom\":\"Москва\",\"cityTo\":\"Уфа\",\"loadAddress\":\"ул. Северная, 54\",\"loadContact\":\"+7 (900) 111-22-33 Пётр\",\"loadDateFrom\":\"2026-09-21\",\"loadTimeFrom\":\"09:00\",\"unloadDateTo\":\"2026-09-22\"}"); check "v23: дата погрузки правится" "r.get('application',{}).get('loadDateFrom')=='2026-09-21'" "$R"
