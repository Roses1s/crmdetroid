...
// --- Теги лидов (v17; свободный цвет — §39) ------------------------------------
// Принимается любой #rrggbb (пресеты для быстрого выбора — TAG_COLORS в js/lead.js).
// Чипы красятся через el.style из JS (CSP разрешает; запрещены лишь style-атрибуты
// в HTML), текст — автоконтраст. Мусор сводится к дефолту (индиго).
/** Цвет тега: валидный #rrggbb (в нижнем регистре) или цвет по умолчанию. */
function crm_tag_color(string $c): string {
    $c = strtolower(trim($c));
    return preg_match('/^#[0-9a-f]{6}$/', $c) ? $c : '#6366f1';
}
